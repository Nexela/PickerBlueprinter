...
local function cycle_tile_mode(event)
    ...
    if stack and stack.valid_for_read and stack.name == 'deconstruction-planner' then
        ...
        stack.tile_selection_mode = next_mode --[[@as defines.deconstruction_item.tile_selection_mode]]
...
local function mark_for_deconstruction(event)
    ...
    if selected then
        local stack = player.cursor_stack
        if stack and stack.is_deconstruction_item then
            if not (selected.type == 'resource' or selected.has_flag('not-deconstructable')) then
                ...
                for i = 1, count do
                    local filter = stack.get_entity_filter(i--[[@as uint]] )
                    if name == filter then
                        stack.set_entity_filter(i--[[@as uint]] , nil) ---@diagnostic disable-line: param-type-mismatch ---@bug apidoc
                ...
                if first_empty > 0 and stack.set_entity_filter(first_empty--[[@as uint]] , name) then
        ...
        elseif not (stack and stack.valid_for_read) then
